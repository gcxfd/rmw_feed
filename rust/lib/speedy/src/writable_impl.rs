...
macro_rules! impl_for_non_zero {
    ($type:ident, $base_type:ty) => {
        impl<C> Writable<C> for std::num::$type
        where
            C: Context,
        {
            #[inline]
...
macro_rules! impl_for_addr {
    ($type:ident) => {
        impl<C> Writable<C> for std::net::$type
        where
            C: Context,
        {
            #[inline]
            ...
            {
                match self {
                    std::net::$type::V4(address) => {
                        writer.write_u8(0)?;
                        address.write_to(writer)
                    }
                    std::net::$type::V6(address) => {
                        writer.write_u8(1)?;
                        address.write_to(writer)
                    }
                }
...
impl_for_addr!(IpAddr);
impl_for_addr!(SocketAddr);

macro_rules! impl_for_socketaddr {
...
impl_for_socketaddr!(SocketAddrV4, 6);
impl_for_socketaddr!(SocketAddrV6, 18);

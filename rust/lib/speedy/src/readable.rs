...
struct CopyingBufferReader<'ctx, 'a, C>
where
    C: Context,
{
    context: &'ctx mut C,
...
impl<'ctx, 'a, C> CopyingBufferReader<'ctx, 'a, C>
where
    C: Context,
{
    #[inline]
...
fn test_peek() {
    ...
    let data = unsafe { std::slice::from_raw_parts(value.as_ptr() as *const u8, 16) };

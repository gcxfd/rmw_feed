use {
    speedy::{
        private::{get_error_kind, ErrorKind},
        Endianness, Readable, Writable,
    },
...
#[static_test]
fn write_u16_to_buffer_when_buffer_length_is_known_and_there_is_enough_space(
    slice: &mut [u8],
    value: u16,
) {
    assume!(slice.len() == 2);
    static_assert!(value
        .write_to_buffer_with_ctx(Endianness::NATIVE, slice)
        .is_ok());
}

#[static_test]
fn write_u16_to_buffer_when_buffer_length_is_known_and_there_is_not_enough_space_1(
    slice: &mut [u8],
    value: u16,
) {
    assume!(slice.len() == 1);
...
#[static_test]
fn write_vec_u8_to_buffer_when_both_lengths_are_known_and_there_is_not_enough_space_1(
    slice: &mut [u8],
    value: Vec<u8>,
) {
    assume!(slice.len() == 5);
...
#[static_test]
fn write_vec_u8_to_buffer_when_both_lengths_are_known_and_there_is_not_enough_space_2(
    slice: &mut [u8],
    value: Vec<u8>,
) {
    assume!(slice.len() == 5);
...
#[static_test]
fn write_vec_u8_to_buffer_when_buffer_length_is_known_and_there_is_not_enough_space_1(
    slice: &mut [u8],
    value: Vec<u8>,
) {
    assume!(slice.len() == 3);
...
#[static_test]
fn write_vec_u8_to_buffer_when_buffer_length_is_known_and_there_is_not_enough_space_2(
    slice: &mut [u8],
    value: Vec<u8>,
) {
    assume!(slice.len() == 3);
